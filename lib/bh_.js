/**
 * Создан на основе https://github.com/bem/bh/blob/master/lib/bh.js
 * Внесены изменения для замены БЭМ-классов на кастомные через config.replacements
 */
var BH = (function() {
...
BH.prototype = {
    ...
    _html: function(json) {
        ...
        if (typeof json !== 'object') {
            ...
            var replaced = (this._options.production ? false : getBemClassesReplaced(cls, this._options.replacements || {}));
            cls = makeBemClassesReplacements(cls, this._options.replacements || {});

            var tag = (json.tag || 'div');
            this._buf += '<' + tag + (cls ? ' class="' + cls + '"' : '') + (replaced ? ' data-bem-repaced="' + replaced + '"' : '') + (attrs ? attrs : '');
...
var makeBemClassesReplacements = function(cls, repl) {
    if (!cls)
        return cls;
...
var getBemClassesReplaced = function(cls, repl) {
    if (!cls)
        return '';
